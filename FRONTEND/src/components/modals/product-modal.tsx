...
const ProductModal = ({ show, handleClose, setRefresh }: Props) => {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<FormValues | any>({ resolver: yupResolver(validationSchema) });
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files) {
     ...
     authAxios.post('/uploads/image', formData,{headers: {
    ...
    <ModalContainer title="Add Product" handleClose={handleClose} show={show}>
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Form.Group>
          ...
          <Form.Control type="file" placeholder="Gtx 1660 super" name="image" onChange={onChange} />
        </Form.Group>
        <Form.Group>
